...
impl<Code: Executable> FunctionPrototype<Code> {
    pub fn call(&self, state: &mut Code::State, stack_ptr: Weak<Vec<Slot>>) {
        let stack = unsafe{&mut*(stack_ptr.as_ptr()).cast_mut()};
        ...
        let mut frame = Frame::new(fp, self.captures.len(), stack_ptr);

...
unsafe impl GlobalAlloc for TaggedPointerAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if layout.align() < 8 {
            return System.alloc(Layout::from_size_align_unchecked(layout.size(), ALIGN))
        }
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        //System.dealloc((ptr as usize & MASK) as *mut u8, layout)
    }
}
...
pub struct Slice<T>{
    pub(crate) words: Ref<u8>, // [len: size_of<usize>(), ...]
...
impl<T> Deref for Slice<T> {
    ...
    fn deref(&self) -> &Self::Target {
        unsafe{from_raw_parts(self.words.as_slice(self.len()).as_ptr().add(size_of::<usize>()) as *const T, self.len())}
    }
}
...
impl<T> IndexMut<usize> for Slice<T> {
    fn index_mut(&mut self, index: usize) -> &mut Self::Output {
        unsafe{&mut*(self.words.as_slice_mut(self.len()).as_mut_ptr().add(size_of::<usize>()+index*size_of::<T>()) as *mut T)}
    }
}
...
impl<T> Ref<T> {
    ...
    */
    ...
    pub fn as_slice_mut(&mut self, len: usize) -> &mut [T] {
        let ptr = unsafe{&mut*(((self.0 as usize) & MASK) as *mut T)};
        //let ptr = &mut unsafe{*self.0};
...
impl<T> Deref for Ref<T> {
    ...
    fn deref(&self) -> &Self::Target {
        ...
        unsafe{&*(((self.0 as usize) & MASK) as *const T)}
    }
}
...
impl<T> DerefMut for Ref<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        ...
        unsafe{&mut*(((self.0 as usize) & MASK) as *mut T)}        
    }
}
/*
